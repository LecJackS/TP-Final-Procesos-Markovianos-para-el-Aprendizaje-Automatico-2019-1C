...
class SimpleActorCriticWithDropout(nn.Module):
    ...
    def forward(self, x, hx, cx):
        ...
        if use_dropout:
            ...
            self.p_drop_conv   = max_drop_conv
            self.p_drop_linear = max_drop_linear
        return actor, critic, hx, cx
...
class Mnih2016ActorCriticWithDropout(nn.Module):
    ...
    def _initialize_weights(self):
        for module in self.modules():
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                #Freezing convs and linear (already trained)
                ...
                # concatenation of (W_ii|W_if|W_ig|W_io)
                nn.init.constant_(module.bias_ih, 0)
                # concatenation of (W_hi|W_hf|W_hg|W_ho)
                nn.init.constant_(module.bias_hh, 0)
                #nn.init.constant_(module.bias_hh, 0)
